...
class FileManagerImpl constructor(
...
//        val directory = File(applicationContext.filesDir, dir)
//        return if (directory.exists() || directory.mkdirs()) {
//            directory
//        } else null
//    }

    override suspend fun createFile(directory: String, ext: String): String {
        return withContext(Dispatchers.IO) {
            val timestamp = SimpleDateFormat(
                FILE_TIMESTAMP_FORMAT,
                Locale.getDefault()
            ).format(System.currentTimeMillis())
            return@withContext File(appStorageRoot, "$timestamp.$ext").canonicalPath
        }
    }
}